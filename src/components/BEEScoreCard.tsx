...
export function BEEScoreCard({ beeResult, employees = [], sessionId = 'default' }: BEEScoreCardProps) {
  ...
  const blackDisabledPercentage = totalEmployees > 0 ? (blackDisabledEmployees.length / totalEmployees) * 100 : 0;
    ...
    const gaps = {
      senior: {
        blackGap: beeResult.seniorManagement.blackPercentage < 60,
        blackFemaleGap: beeResult.seniorManagement.blackFemalePercentage < 30,
      ...
      middle: {
        blackGap: beeResult.middleManagement.blackPercentage < 75,
        blackFemaleGap: beeResult.middleManagement.blackFemalePercentage < 38,
      ...
      junior: {
        blackGap: beeResult.juniorManagement.blackPercentage < 88,
        blackFemaleGap: beeResult.juniorManagement.blackFemalePercentage < 44,
  ...
  const generateSpecificRecommendations = (gaps: any) => {
    ...
    if (gaps.senior?.blackGap && gaps.senior.current > 0) {
    ...
    if (gaps.senior?.blackFemaleGap && gaps.senior.current > 0) {
    ...
    if (gaps.middle?.blackGap && gaps.middle.current > 0) {
    ...
    if (gaps.middle?.blackFemaleGap && gaps.middle.current > 0) {
    ...
    if (gaps.junior?.blackGap && gaps.junior.current > 0) {
    ...
    if (gaps.junior?.blackFemaleGap && gaps.junior.current > 0) {
